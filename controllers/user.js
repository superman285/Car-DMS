...
module.exports = {
    ...
    login: async (params)=>{
        ...
        if (result.length) {
            ...
            let [encryPhone,encryPwd,encryID,name] = [encrypt(String(result[0].phone)),encrypt(String(result[0].password)),encrypt(String(result[0].id)),result[0].name];
            console.log(encryPhone,encryPwd,encryID,result[0].name);
            return [encryPhone,encryPwd,encryID,name];
        }else {
            return false;
        }
    },
